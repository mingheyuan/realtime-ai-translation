...
打开 <http://127.0.0.1:8765>，选择音频来源和翻译方向，再点击“开始实时翻译”。点击“悬浮字幕”会打开一个不占 Dock、半透明、始终置顶的原生字幕窗口；窗口内可以直接开始或停止翻译，并沿用主界面打开悬浮窗时选择的音频来源和翻译方向。拖动顶部可以移动，拖动窗口边缘可以缩放，位置和尺寸会自动记忆，点右上角 `×` 关闭。它和主界面读取同一个 WebSocket，不会启动第二套 ASR 或翻译任务。
